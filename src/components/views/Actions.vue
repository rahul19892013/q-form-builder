...
export default {
  ...
  data: function () {
    return {
      ...
      columns: [
        ...
        { name: 'subCategoryName', align: 'left', label: 'Sub-Category', field: 'subCategoryName', sortable: true },
  ...
  methods: {
    ...
    getSubCategories() {
      axios.get(APISettings.baseURL + 'SubCategory/GetSubCategories?CategoryId=' + 1,
        {
          headers: APISettings.getHeaders()[0]
          //  ,params: APISettings.axiosParams(3, 7)
        }).then(res => {
          this.subCategories = res.data.data.records
        }).catch(e => {
          this.showError(e)
        })
    },
    ...
    <q-page>
      <div id="q-app">
        <div class="q-pa-sm q-gutter-sm">
          <q-table title="Actions" :data="data" :columns="columns" row-key="id" :filter="filter" binary-state-sort>
            <template v-slot:top-right>
              <q-tr>
                ...
                <q-td></q-td>
                <q-td>
                  <q-btn flat outline dense color="primary" label="Add Action"
                    style="background-color: #027be3; color:#ffffff !important;padding:4px;"
                    @click="showAddItemDialog('Add Action')"></q-btn>
                </q-td>
              </q-tr>
              <div class="q-pa-sm q-gutter-sm">
                    ...
                    <q-card-section>
                      <div class="row">
                        <q-form @submit="onSubmit" @reset="onReset">
                          <q-input v-model="editedItem.name" label="Action Name" lazy-rules
                            :rules="[val => val && val.length > 0 || 'Please Enter Action Name']" maxlength="100">
                            <q-avatar>
                          ...
                          <q-select filled bottom-slots v-if="dialogHeaderText == 'Add Action'"
                            v-model="editedItem.categoryId" emit-value map-options options-dense option-label="name"
                            option-value="id" :options="categories" label="Select Category *"
                            @update:model-value="getSubCategories()" lazy-rules
                          ...
                          <q-select filled bottom-slots v-if="dialogHeaderText == 'Add Action'"
                            v-model="editedItem.categoryName" emit-value map-options options-dense option-label="name"
                            option-value="id" :options="subCategories" label="Select Sub-Category *" lazy-rules
                            :rules="[val => val || 'Please select sub-category']">
                            <template v-slot:prepend>
                              <q-icon name="category" @click.stop.prevent />
                            </template>
                            <template v-slot:append>
                          ...
                          <div v-if="dialogHeaderText == 'Update Action'">
                            <q-avatar>
                              <q-icon name="category" color="black" />
                            </q-avatar>
                            Category: {{ editedItem.categoryName }}
                          </div>
                          <br v-if="dialogHeaderText == 'Update Action'" />
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="subCategoryName" :props="props">
                  {{ props.row.subCategory.name }}
                </q-td>
                <q-td key="categoryName" :props="props">
                  {{ props.row.subCategory.categoryName }}
                </q-td>
                ...
                <q-td key="actions" :props="props">
                  <div style="float: right;">
                    <q-btn color="blue" label="update" @click="showEditItemDialog(props.row, 'Update Action')" size=sm>
